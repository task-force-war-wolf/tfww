...
if (!isNil "ace_medical_treatment_fnc_fullHeal") then {
    {
        ...
        (format ["[Task Force War Wolf  Admin Menu] Your health was restored by '%1'", profileName]) remoteExec ["systemChat", _x];
...
systemChat format ["[Task Force War Wolf  Admin Menu] %1 players had their health restored", count GVAR(utilityData)];
