...
private _presetName = format ["tfww_preset%1", _network];
...

{
    if (player canAdd _radio) then {
        ...
        systemChat format ["[Task Force War Wolf ] Added radio: %1", _radio];
    } else {
        if (getContainerMaxLoad uniform player > 0) then {
            (uniformContainer player) addItemCargoGlobal [_radio, 1];
            systemChat format ["[Task Force War Wolf ] Added radio (exceeds inventory capacity): %1", _radio];
        } else {
            systemChat format ["[Task Force War Wolf ] Couldn't add radio: %1", _radio];
        };
