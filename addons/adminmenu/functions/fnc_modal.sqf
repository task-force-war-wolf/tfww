...
if (isNil _utilityFunction) exitWith {
    systemChat format ["[Task Force War Wolf  Admin Menu] Modal utility with name '%1' requires undefined function '%2'", _utilityName, _utilityFunction];
};

GVAR(utilityData) = [];
private _exit = false;

if ((missionNamespace getVariable [QGVAR(selectedTab), -1]) isEqualTo IDC_TFWW_ADMINMENU_G_PMAN) then {
...
if (_exit) exitWith {
    systemChat "[Task Force War Wolf  Admin Menu] No valid player(s) selected for the action!";
};
