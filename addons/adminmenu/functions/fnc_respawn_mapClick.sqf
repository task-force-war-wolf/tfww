...
if (_type == 0) then { // left click
    ...
    private _var = missionNamespace getVariable[format["tfww_respawnPoint%1",_i],objNull];
    while {!(isNull _var)} do {
        ...
        _i = _i + 1;
        _var = missionNamespace getVariable[format["tfww_respawnPoint%1",_i],objNull];
    };
    if (!_found) then {
