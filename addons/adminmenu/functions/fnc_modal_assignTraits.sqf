...
_ctrlLabelTraits ctrlSetPosition [0, 0, (0.25 * _ctrlGrpWidth), TFWW_ADMINMENU_STD_HEIGHT];
...
private _ctrlComboTraitY = 2.2 * TFWW_ADMINMENU_STD_HEIGHT;
private _ctrlComboTraitW = 0.5 * _ctrlGrpWidth;
private _traitComboCtrls = [];

{
    private _ctrlLineY = _ctrlComboTraitY + _forEachIndex * (1.1 * TFWW_ADMINMENU_STD_HEIGHT);
    ...
    _ctrlComboTraitLabel ctrlSetPosition [0.1 * TFWW_ADMINMENU_STD_WIDTH, _ctrlLineY, _ctrlComboTraitLabelW, TFWW_ADMINMENU_STD_HEIGHT];
    ...
    _ctrlComboTrait ctrlSetPosition [_ctrlComboTraitX, _ctrlLineY, _ctrlComboTraitW, TFWW_ADMINMENU_STD_HEIGHT];
...
_ctrlButton ctrlAddEventHandler ["ButtonClick", {
    ...
    systemChat format ["[Task Force War Wolf  Admin Menu] Assigned traits to %1 player(s)", count GVAR(utilityData)];
